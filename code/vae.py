...
flags.DEFINE_integer("input_size", 64, "size of the image")
...
def encoder(input_tensor):
    ...
    #a = pt.wrap(input_tensor).reshape([FLAGS.batch_size, FLAGS.input_size, FLAGS.input_size, 1])
    #b = a.conv2d(5, 32, stride=2)
    #c = b.conv2d(5, 64, stride=2)
    #d = c.conv2d(5, 128, stride=2)
    #e = d.conv2d(5, 256, edges='VALID')
    #print (a.get_shape())
    #print (b.get_shape())
    #print (c.get_shape())
    #print (d.get_shape())
    ...
    return (pt.wrap(input_tensor).
            reshape([FLAGS.batch_size, FLAGS.input_size, FLAGS.input_size, 1]).
...
if __name__ == "__main__":
    ...
    input_tensor = tf.placeholder(tf.float32, [FLAGS.batch_size, FLAGS.input_size * FLAGS.input_size])
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(FLAGS.max_epoch):
            ...
            for i in range(FLAGS.updates_per_epoch):
                pbar.update(i)
                x, _ = mnist.train.next_batch(FLAGS.batch_size, FLAGS.input_size)
                for s in range(len(x)):
                    print('Size of X:'+str(len(x[s])))
                    imgs_folder = os.path.join(FLAGS.working_directory, 'input')
                    if not os.path.exists(imgs_folder):
                        os.makedirs(imgs_folder)
                    imsave(os.path.join(imgs_folder, '%d_%d.png') % (i,s),
                       x[s].reshape(FLAGS.input_size, FLAGS.input_size))
                _, loss_value, output = sess.run([train, loss, output_tensor], {input_tensor: x})
                for r in range(len(output)):
                    imgs_folder = os.path.join(FLAGS.working_directory, 'output')
                    if not os.path.exists(imgs_folder):
                        os.makedirs(imgs_folder)
                    imsave(os.path.join(imgs_folder, '%d_%d.png') % (i,r),
                       output[r].reshape(FLAGS.input_size, FLAGS.input_size))
                training_loss += loss_value

            training_loss = training_loss / \
                (FLAGS.updates_per_epoch * FLAGS.input_size * FLAGS.input_size * FLAGS.batch_size)
            ...
            for k in range(FLAGS.batch_size):
                ...
                imsave(os.path.join(imgs_folder, '%d.png') % k,
                       imgs[k].reshape(FLAGS.input_size, FLAGS.input_size))
