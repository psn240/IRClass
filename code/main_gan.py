...
flags.DEFINE_string("working_directory", "tmp/gan", "")
flags.DEFINE_integer("input_size", 28, "size of the image")
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(FLAGS.max_epoch):
            ...
            for i in range(FLAGS.updates_per_epoch):
                ...
                #    imgs_folder = os.path.join(FLAGS.working_directory, 'input-gan')
		...
		#	    imsave(os.path.join(imgs_folder, '%d_%d.png') % (i,s),
        	#                x[s].reshape(FLAGS.input_size, FLAGS.input_size))
                # end - code addition
                _, loss_value, output = sess.run([train_generator, G_loss, G], {input_tensor: x, learning_rate: FLAGS.g_learning_rate})

                # start - code addition
                #for r in range(len(output)):
                #    imgs_folder = os.path.join(FLAGS.working_directory, 'output-gan')
                #    if not os.path.exists(imgs_folder):
                #        os.makedirs(imgs_folder)
                #    if i % 100 ==0:
		#	if r == 127:
		#    		imsave(os.path.join(imgs_folder, '%d_%d.png') % (i,r),
            ...
            for k in range(FLAGS.batch_size):
                imgs_folder = os.path.join(FLAGS.working_directory, 'imgs-gan')
                if not os.path.exists(imgs_folder):
                    os.makedirs(imgs_folder)

                imsave(os.path.join(imgs_folder, '%d_%d.png') % (epoch,k),
